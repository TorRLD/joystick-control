/**
 * Código completo para o projeto BitDogLab com calibração do eixo Y:
 * - Controle de LEDs RGB via PWM com base no joystick (ADC) com deadzone, filtragem e calibração.
 * - Exibição de um quadrado móvel e borda com estilos alternados no display SSD1306.
 * - Uso de interrupções (com debouncing) para os botões do joystick e botão A.
 */
